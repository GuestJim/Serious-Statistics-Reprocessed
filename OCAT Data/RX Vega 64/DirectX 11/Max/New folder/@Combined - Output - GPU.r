...
for (GPU in listGPU) {	if (file.exists(GPU))	{
	options(width = 1000)
	sink(paste0(GPU, "\\", gameGAQF, " Frame Data.txt"), split = TRUE)
		writeLines(paste0(gameGAQ))
		writeLines("Frame Time")
		writeLines("\nMean")
		print(addFPS(dataMEAN[dataMEAN$GPU==GPU,]), row.names = FALSE)
		writeLines("\nPercentiles")
		print(addFPS(dataPERC[dataPERC$GPU==GPU,]), row.names = FALSE)
		writeLines("\nPercentile of FPS")
...
for (Location in listLOC) {
	options(width = 1000)
	sink(paste0(gameGAQF, " - ", Location, " Frame Data.txt"), split = TRUE)
		writeLines(game)
		writeLines("Frame Time")
		writeLines("\nMean")
		print(addFPS(dataMEAN[dataMEAN$Location==Location,]), row.names = FALSE)
		writeLines("\nPercentiles")
		print(addFPS(dataPERC[dataPERC$Location==Location,]), row.names = FALSE)
		writeLines("\nPercentile of FPS")
		print(dataECDF[dataECDF$Location==Location,], row.names = FALSE)
		writeLines("\nDistribution Stats")
		print(dataSTAT[dataSTAT$Location==Location,], row.names = FALSE)
	sink()
	}	}
}
...
if (textDISP){
	options(width = 1000)
	sink(paste0(gameGAQF, " Display Data.txt"), split = TRUE)
	writeLines(paste0(gameGAQ))
	writeLines("Display Time")
	writeLines("\nMean")
	print(addFPS(dispMEAN), row.names = FALSE)
	writeLines("\nPercentiles")
	print(addFPS(dispPERC), row.names = FALSE)
	writeLines("\nPercentile of FPS")
	...
	for (GPU in listGPU) {	if (file.exists(GPU))	{
	options(width = 1000)
	sink(paste0(GPU, "\\", gameGAQF, " Display Data.txt"), split = TRUE)
		writeLines(paste0(gameGAQ))
		writeLines("Display Time")
		writeLines("\nMean")
		print(addFPS(dispMEAN[dispMEAN$GPU==GPU,]), row.names = FALSE)
		writeLines("\nPercentiles")
		print(addFPS(dispPERC[dispPERC$GPU==GPU,]), row.names = FALSE)
		writeLines("\nPercentile of FPS")
	...
	for (Location in textLOC) {
		...
		sink(paste0(gameGAQF, " - ", Location, " Frame Data.txt"), split = TRUE)
			...
			writeLines("Frame Time")
			writeLines("\nMean")
			print(addFPS(dispMEAN[dispMEAN$Location==Location,]), row.names = FALSE)
			writeLines("\nPercentiles")
			print(addFPS(dispPERC[dispPERC$Location==Location,]), row.names = FALSE)
			writeLines("\nPercentile of FPS")
...
if	(textLOC){
	for	(Location in textLOC)	{
	writeSUB(addFPS(dataMEAN), Location, typeName = "dataMEAN")
	writeSUB(addFPS(dataPERC), Location, typeName = "dataPERC")
...
for (GPU in listGPU) {	if (file.exists(GPU))	{
	writeGPU(addFPS(dataMEAN), GPU, typeName = "dataMEAN")
	writeGPU(addFPS(dataPERC), GPU, typeName = "dataPERC")
...
if (textDISP){
writeOCC(addFPS(dispMEAN), typeName = "dispMEAN")
writeOCC(addFPS(dispPERC), typeName = "dispPERC")
writeOCC(dispECDF)
writeOCC(dispSTAT)
...
if	(textLOC){
	for	(Location in textLOC)	{
	writeSUB(addFPS(dispMEAN), Location, typeName = "dispMEAN")
	writeSUB(addFPS(dispPERC), Location, typeName = "dispPERC")
	writeSUB(dispECDF, Location)
	writeSUB(dispSTAT, Location)
	writeSUB(compTAB(dispMEAN, dispPERC, dispECDF), Location, typeName = "dispCOMP")
	}	}

for (GPU in listGPU) {	if (file.exists(GPU))	{
	writeGPU(addFPS(dispMEAN), GPU, typeName = "dispMEAN")
	writeGPU(addFPS(dispPERC), GPU, typeName = "dispPERC")
...
graphMEANS = function(datatype)	{
	...
	ggplot(data = results) +
	ggtitle(gameQ, subtitle = paste0(datatype, " - Means, Medians, and Percentiles")) + labs(caption = cGPU) +
...
graphCOURSE = function(datatype)	{
	...
	ggtitle(gameQ, subtitle = paste0(datatype, " - Course")) + labs(caption = cGPU) +
	geom_hline(yintercept = 1000/60, color = "red") +
...
graphDIFF	=	function(datatype)	{
	...
	ggtitle(gameQ, subtitle=paste0(datatype, " consecutive differences")) + labs(caption = cGPU) +
...
graphFREQ	=	function(datatype)	{
	...
	ggtitle(gameQ, subtitle=paste0(datatype, " - Frequency Plot")) + labs(caption = cGPU) +
...
graphQQ	=	function(datatype)	{
	...
	ggplot() +
	ggtitle(gameQ, subtitle=paste0(datatype, " - QQ Distribution")) + labs(caption = cGPU) +
	geom_hline(yintercept = 1000/60, color = "red") +
